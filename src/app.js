...
import { default as connectDB } from './databases/index.js';
import { queryMapper, responseInterceptor } from './middleware/index.js';
import { default as routes } from './routes/index.routes.js';
const { omit, pick } = pkg;
...
app.get('/', (_, res) => res.status(200).json({ message: 'Core Server Up and Running' }));
...
// global.__basedir = __dirname.cjs;

const port = process.env.PORT || 3000;

app.listen(port, (err) => {
    if (!err) {
        logger.info(`Core server successfully started on port ${port}`);
    }
});
